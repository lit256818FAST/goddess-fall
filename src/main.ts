...
function renderCampaignHome(){
  ...
  set(`<main class="campaign-shell hud-page">${flowHeader('home')}<div class="campaign-home-grid"><section class="campaign-map" aria-label="战斗路线"><img src="/assets/images/campaign-route-map.webp" alt="从圣辉城通向铁砧边境与河湾境的战斗路线地图" width="1600" height="1000">${nodes}</section><aside class="campaign-next"><div class="campaign-next-heading"><p class="eyebrow">下一行动</p><h1>${escapeHtml(route.title)}</h1><p class="location">${escapeHtml(route.location)} · 风险${escapeHtml(visibleRisk())}</p></div><figure class="campaign-next-scene"><img src="/assets/images/campaign-route-map.webp" alt="${escapeHtml(route.title)}所在区域的路线图景" width="1600" height="1000"><figcaption><span>当前战斗节点</span><strong>${escapeHtml(route.location)}</strong></figcaption></figure><div class="campaign-status-line"><span>平民安全 <b>${campaignState.civilianSafety}/3</b></span><span>${campaignState.campaignId==='arthur-main'?'军队声望':'女神国'} <b>${campaignState.campaignId==='arthur-main'?campaignState.factionAttitudes.wardens:attitudeLabel(campaignState.factionAttitudes.goddessState)}</b></span><span class="campaign-lineup-status">当前阵容 <b>${escapeHtml(lineupNames)}</b></span></div>${mainlineStats}<p>${escapeHtml(route.objective)}</p><section class="campaign-actions"><div class="campaign-actions-heading"><h2>${campaignState.campaignId==='arthur-main'?'战前准备':'准备行动'}</h2><span>三选一</span></div>${actions}</section>${mainlineMissionPanel}<button id="open-battle" class="campaign-primary">查看战斗</button><p id="home-feedback" class="campaign-feedback" aria-live="polite"></p></aside></div><section class="home-lineup"><header><p class="eyebrow">出战三人</p><button id="home-roster" class="text-button">调整阵容</button></header><div>${lineup}</div></section></main>`);
