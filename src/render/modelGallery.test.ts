...
describe('model gallery maker classification', () => {
  it('separates K3, GPT/img2threejs, and external project assets', () => {
    ...
    expect(modelGalleryAssets.filter(asset => asset.maker === 'GPT').every(asset => asset.url.includes('/model-inventory/assets/models/') && asset.url.includes('/mainline/'))).toBe(true);
  });
});
