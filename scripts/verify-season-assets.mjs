...
const inventoryRoot=resolve('model-inventory');
const required=[
...
const missing=required.filter(path=>!existsSync(join(inventoryRoot,path)));
...
const lazyMusicRoot=join(resolve('public'),'assets','audio','music-lazy');
...
for(const chapter of ['shenghui','anvil-frontier','veil-sanctum']){
  const manifestPath=join(inventoryRoot,`assets/models/environment/${chapter}/manifest.json`);
  ...
  for(const entry of entries){
    ...
    const modelPath=join(inventoryRoot,`assets/models/environment/${sourceChapter}`,entry.file);
...
for(const hero of ['unflagged','seraphina','reina','odric','cole','agnes']){
  const bytes=readFileSync(join(inventoryRoot,`assets/models/heroes/${hero}.glb`));
